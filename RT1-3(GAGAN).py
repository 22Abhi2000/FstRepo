...
def connect_menu_init():
    ...
    tab_notebook.grid(row=1, column=0, sticky="nsew", columnspan=2)
    tab_frames = {}
    ...
    for rt_name in ["RT1", "RT2", "RT3"]:
        root = tk.Frame(tab_notebook, bg="burlywood")
        tab_notebook.add(root, text=rt_name)
        tab_frames[rt_name] = root
...
def readSerial():
    ...
    try:
        while serialData:
            if ser.in_waiting > 0:
                byte = ser.read(1).hex()
                if byte == headers[header_index]:
                    header_index += 1
                    if header_index == len(headers):
                        print(f"{selected_tab} header found! reading data")
                        hexDecodedData = ''.join(headers) + ser.read(1206).hex()
                        # You can use selected_tab as data_type to know which RT
...
def process_data():
    ...
    try:
        while True:
            ...
            headers = RT_HEADERS[selected_tab]
            main_header = headers[0]  # e.g. acca1f0a
            secondary_header = headers[1]  # e.g. acca1f0b
...
def process_replay_file(filepath):
    ...
    secondary_header = headers[1] if len(headers) > 1 else None
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if jump_target_sec is not None:
                    if SYS_Second < jump_target_sec: continue
                    elif SYS_Second > jump_target_sec: continue
                    else:
                ...
                if hexDecodedData.startswith(main_header):
                    data_queue.put((selected_tab, hexDecodedData))
                    print(f"{selected_tab} - {hexDecodedData[:32]}...")
                elif secondary_header and hexDecodedData.startswith(secondary_header):
                    data_queue.put((selected_tab, hexDecodedData))
                    print(f"{selected_tab} (secondary) - {hexDecodedData[:32]}...")
